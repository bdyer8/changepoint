...
%  dimensions=linspace(range(1),range(end)-.01,10000);
 %unZ
 for i=1:1:size(compModel.mFit,2)
    compModel.mFit(:,i)=compModel.mFit(:,i)*zSigma(i)+zMean(i);
    compModel.mFitLow(:,i)=compModel.mFitLow(:,i)*zSigma(i)+zMean(i);
    compModel.mFitHigh(:,i)=compModel.mFitHigh(:,i)*zSigma(i)+zMean(i);
    compModel.sigma(:,i)=compModel.sigma(:,i)*zSigma(i);
    data(:,i)=compModel.data(:,i)*zSigma(i)+zMean(i);
 end
...
legendStr(i)={num2str(i)};
    ...
    formatPlot(gca,'Data (\delta)','Meters','');
